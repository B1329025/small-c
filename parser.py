...
class Parser:
    ...
    def parse_statement(self):
        ...
        if token.type == 'LBRACES': 
            ...
            return block_node
        if token and token.type in ('INT','CHAR'):
    ...
    def parse_printf(self):
        self.eat('PRINTF') # 吃掉 'printf'
        self.eat('LPAREN')
        expr = self.logical_or() # 解析括號內的內容
        self.eat('RPAREN')
        self.eat('END') # 吃掉分號 ;
    ...
    def factor(self):
        ...
        if self.current_token().type == 'STRING':
            s_token = self.eat('STRING')
            return StringNode(s_token.value[1:-1])
        if self.current_token().type == 'CHAR':
            char_token = self.eat('CHAR')
            # char_token.value 可能是 "'A'"
            # 我們取中間那個字元並轉成 ASCII 碼
            char_val = ord(char_token.value[1]) 
            return NumberNode(char_val)
